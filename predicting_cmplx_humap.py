...
def network():
    ...
    for n in nodes_list:
        ...
        tup_cmplx = (nodes_order,final_dens,cmplx_val_fn)
        all_lists.append(nodes_order)
        cmplx_info.append(tup_cmplx)
...
delayed(network)(node,scaler, par_inputs_fn) for node in tqdm(all_humap_nodes))

network()
print(cmplx_info)
with open('predicted_complexes humap.txt','w') as f:
    f.writelines([' '.join(comp) + '\n' for comp in all_lists])
with open('predicted_complexes.pkl', 'wb') as f:
    pickle_dump(all_lists, f)
with open('humap_CORUM_complexes_node_lists.pkl','wb') as f:
    pickle_dump(complexes,f)
with open('predicted_complexes density and value function humap.txt','w') as f:
    for i in cmplx_info:
        f.writelines(', '.join(str(comp) for comp in i) + '\n')
